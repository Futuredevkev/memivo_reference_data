 ...
 * Esta lista estaba escrita a mano y decía DIECISIETE cuando `src/` tenía
 * VEINTE: le faltaban `billing`, `moderation` y `stickers`. Todo lo que este
 * archivo afirma «para todos los dominios» —que cada uno tenga su barrel, entre
 * otras— pasaba en verde sobre tres que nunca miró, y el más nuevo era
 * justamente el que más superficie agregó.
 *
...
const SORTED_BARRELS = [
  ...
  'auth/enums/index.ts',
  'auth/index.ts',
  'billing/constants/index.ts',
  'billing/enums/index.ts',
  'billing/index.ts',
  'billing/interfaces/index.ts',
  ...
  'mentions/constants/index.ts',
  'mentions/helpers/index.ts',
  'mentions/index.ts',
  'mentions/interfaces/index.ts',
  'mentions/types/index.ts',
 ...
 * estaba escrita a mano: listaba CATORCE dominios de los veinte que existen.
 * Faltaban seis, y uno era `billing/` —tres enums, siete interfaces, la tabla de
 * topes del producto y su propio subpath—, o sea la superficie más grande que
 * una ola agregó, invisible en la única página que describe el paquete.
 *
...
test('el bloque de estructura del README nombra exactamente los dominios de `src/`', () => {
  ...
  const declarados = [
    ...bloque[1].matchAll(/^\s{2}([a-z-]+)\/\s/gm),
  ].map((fila) => fila[1]);
