import { validatedTextLength } from './validated-text-length.rule';
 ...
 * puede cortar.
 */
export const cutToValidatedLength = (text: string, max: number): string => {
  if (max <= 0) return '';
  if (validatedTextLength(text) <= max) return text;

  const esSelector = (unidad: number): boolean =>
    unidad === 0xfe0f || unidad === 0xfe0e;
