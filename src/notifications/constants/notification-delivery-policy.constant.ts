...
// Import directo a los archivos y no al barrel: `interfaces/index.ts` reexporta
// funciones que consumen ESTA constante como valor, y pasar por el barrel
// crearía un ciclo de módulos en CommonJS aunque acá sólo se usen tipos.
import type { NotificationContextIdSource } from '../interfaces/notification-context-id-source.type';
import type { NotificationDeliveryPolicy } from '../interfaces/notification-delivery-policy.interface';
import type { NotificationForegroundRedundancy } from '../interfaces/notification-foreground-redundancy.type';
import type { NotificationInAppSurface } from '../interfaces/notification-in-app-surface.type';

/**
 * En chat el grupo viaja en metadata; el `resourceId` es el mensaje. Se mira
 * metadata primero y el recurso como respaldo.
 */
const CHAT_ID_SOURCES: readonly NotificationContextIdSource[] = [
  'metadata.groupId',
  'resourceId',
];

/** En posts el `resourceId` YA es el post; metadata es el respaldo. */
const POST_ID_SOURCES: readonly NotificationContextIdSource[] = [
  'resourceId',
 ...
 */
const REACTION_POST_ID_SOURCES: readonly NotificationContextIdSource[] = [
  'metadata.guestPostId',
];

/** En historias el id viaja en metadata; el recurso puede ser el comentario. */
const STORY_ID_SOURCES: readonly NotificationContextIdSource[] = [
  'metadata.storyId',
  'resourceId',
];
 ...
 */
const ALBUM_ID_SOURCES: readonly NotificationContextIdSource[] = [
  'metadata.albumId',
  'resourceId',
];
...
const policy = (row: {
  readonly viewing?: {
    readonly key: ActiveViewKey;
    readonly from: readonly NotificationContextIdSource[];
  };
...
export const NOTIFICATION_DELIVERY_POLICY: Readonly<
  Record<NotificationType, NotificationDeliveryPolicy>
> = {
  // ───────────────────────────────────────────────────────────────────────
  // Interacción sobre un post. Redundantes sólo si tenés ESE post abierto.
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.LIKE_PHOTO]: policy({
  ...
  [NotificationType.REPLY_RESPONSE]: policy({
    viewing: { key: 'postId', from: POST_ID_SOURCES },
    foreground: 'none',
    bellRow: true,
    replacedBy: 'post-detail',
  ...
  [NotificationType.COMMENT_ON_COMMENTED_POST]: policy({
    viewing: { key: 'postId', from: POST_ID_SOURCES },
    foreground: 'none',
    bellRow: true,
    replacedBy: 'post-detail',
  ...
  [NotificationType.REPLY_ON_COMMENTED_POST]: policy({
    viewing: { key: 'postId', from: POST_ID_SOURCES },
    foreground: 'none',
    bellRow: true,
    replacedBy: 'post-detail',
  ...
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.NEW_CHAT_MESSAGE]: policy({
  ...
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.TAGGED_IN_STORY]: policy({
  ...
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.MEMIVO_MOMENTS]: policy({
  ...
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.DAILY_MOTIVATIONAL]: policy({
  ...
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.GUEST_POST_UPLOAD_READY]: policy({
  ...
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.ALBUM_DELETED]: policy({
  ...
  // ───────────────────────────────────────────────────────────────────────
  [NotificationType.CHAT_MEMBER_KICKED]: policy({
    viewing: { key: 'chatGroupId', from: CHAT_ID_SOURCES },
    foreground: 'none',
    bellRow: true,
    replacedBy: 'chat-room',
   ...
   */
  [NotificationType.PROFESSIONAL_PHOTOS_UPLOADED]: policy({
    foreground: 'none',
    bellRow: true,
    replacedBy: null,
    anonymousActor: false,
  ...
  [NotificationType.ALBUM_QR_CODE_EXPIRING]: policy({
  ...
  [NotificationType.ALBUM_SUSPENDED_BY_MEMIVO]: policy({
  ...
  [NotificationType.ALBUM_REINSTATED_BY_MEMIVO]: policy({
  ...
  [NotificationType.WARNING_ISSUED_BY_MEMIVO]: policy({
  ...
  [NotificationType.PLAN_EXPIRING_SOON]: policy({
