import { MENTIONS_MAX_ITEMS } from '../../validation/limits/mentions-max-items.constant';
 ...
 */
export const findMentionAnnotationsDefect = (
  text: string,
  annotations: readonly MentionAnnotation[],
): MentionAnnotationsDefect | null => {
  if (annotations.length > MENTIONS_MAX_ITEMS) return 'too-many';
  ...
  for (const { start, length } of annotations) {
    if (
      ...
      start < 0 ||
      length < MENTION_TRIGGER.length + 1
    ) {
      return 'malformed';
    }
    if (start + length > text.length) return 'out-of-bounds';
    if (!text.startsWith(MENTION_TRIGGER, start)) return 'not-on-trigger';
    if (start < previousEnd) return 'unordered-or-overlapping';
