...
import { ChatEditableContent, ChatMutationRefusal } from '../enums';
import type { MutableChatMessage } from '../interfaces';
...
export const canEditChatMessage = (
  ...
  if (!viewerId || message.senderId !== viewerId) {
    return { allowed: false, refusal: ChatMutationRefusal.NOT_THE_AUTHOR };
  }

  if (
    CHAT_MESSAGE_MUTATION_BY_TYPE[message.type].edits ===
    ChatEditableContent.NONE
  ) {
    return { allowed: false, refusal: ChatMutationRefusal.CONTENT_IS_IMMUTABLE };
  }
