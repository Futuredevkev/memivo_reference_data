...
exports.resolveAlbumPosting = void 0;
const album_posting_closed_reason_enum_1 = require("../enums/album-posting-closed-reason.enum");
const album_posting_mode_enum_1 = require("../enums/album-posting-mode.enum");
/**
 ...
 */
const resolveAlbumPosting = (schedule, now) => {
    switch (schedule.mode) {
        case album_posting_mode_enum_1.AlbumPostingMode.EVERYONE:
            return { closedReason: null, nextChangeAt: null, openedAt: null };
        case album_posting_mode_enum_1.AlbumPostingMode.ORGANIZERS_ONLY:
            return {
                closedReason: album_posting_closed_reason_enum_1.AlbumPostingClosedReason.ORGANIZERS_ONLY,
                nextChangeAt: null,
                openedAt: null,
            };
        case album_posting_mode_enum_1.AlbumPostingMode.ONE_SHOT: {
            const at = now.getTime();
            if (at < schedule.opensAt.getTime()) {
                return {
                    closedReason: album_posting_closed_reason_enum_1.AlbumPostingClosedReason.NOT_OPEN_YET,
            ...
            return {
                closedReason: album_posting_closed_reason_enum_1.AlbumPostingClosedReason.ALREADY_CLOSED,
                nextChangeAt: null,
                openedAt: null,
            };
        }
        case album_posting_mode_enum_1.AlbumPostingMode.DAILY: {
            const { timezone } = schedule;
            const today = wallClockIn(timezone, now);
            const isOpen = isWithinDailyWindow(today.hour * 60 + today.minute, schedule);
            return {
                closedReason: isOpen ? null : album_posting_closed_reason_enum_1.AlbumPostingClosedReason.NOT_OPEN_YET,
...
exports.resolveAlbumPosting = resolveAlbumPosting;
/**
 ...
 */
const isWithinDailyWindow = (minuteOfDay, window) => window.opensAtMinute < window.closesAtMinute
 ...
 */
const wallClockIn = (timeZone, instant) => {
    ...
    const read = (type) => {
        ...
        if (!found) {
            throw new Error('El runtime no devolvió el campo "' +
                type +
                '" de la zona "' +
                timeZone +
 ...
 */
const instantOfWallMinute = (timeZone, date, minuteOfDay) => {
    const asIfUtc = Date.UTC(date.year, date.month - 1, date.day, Math.floor(minuteOfDay / 60), minuteOfDay % 60);
    const candidates = [
 ...
 */
const lastDailyOpening = (window, now, today) => {
    let day = today;
    for (let back = 0; back < MAX_DAYS_LOOKING_FOR_THE_NEXT_CHANGE; back++) {
        const candidate = instantOfWallMinute(window.timezone, day, window.opensAtMinute);
        if (candidate.getTime() <= now.getTime())
            return candidate;
        const previousDay = new Date(Date.UTC(day.year, day.month - 1, day.day - 1));
        day = {
 ...
 */
const nextDailyStateChange = (window, now, today, isOpenNow) => {
    ...
    for (let ahead = 0; ahead < MAX_DAYS_LOOKING_FOR_THE_NEXT_CHANGE; ahead++) {
        ...
        if (candidate.getTime() > now.getTime()) {
            ...
            const isOpenThere = isWithinDailyWindow(wall.hour * 60 + wall.minute, window);
            if (isOpenThere !== isOpenNow)
                return candidate;
        }
